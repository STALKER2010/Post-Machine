	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState)
	{
		View v = inflater.inflate(R.layout.editor, container, false);
		{
			...
			btn_run.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View p1)
					{
						if (changedCode && current().debugMode)
						{
							...
							current().reset().load(code.getText().toString(), true);
							code.setText(current().writeCode());
						}
						else if (!current().debugMode)
						{
							currentDoc().linter.messages.clear();
							current().reset().load(code.getText().toString(), true);
							code.setText(current().writeCode());
